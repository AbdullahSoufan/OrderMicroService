...
class bookList(APIView) :
    def put(self, request, id):
        books_with_specific_topic = requests.get('http://127.0.0.1:8001/info/'+str(id)+'/', params=request.GET, timeout=5)
        ...
        if quantity > 0:
            requests.put('http://127.0.0.1:8001/decrease/' + str(id) + '/', timeout=5)
